  ...
  //get OET Images folder (maybe this is unecessary, if ID never changes)
	...
	if (files.length == 0) {
	  console.log('No files found.');
		...
		//get project folders
		service.files.list({
		q: "mimeType='application/vnd.google-apps.folder' and '" + mainFolderID + "' in parents",
		auth: auth,
		pageSize: 10,
		fields: "nextPageToken, files(name, id)"
	  }, function(err, response) {
		...
		if (files.length == 0) {
		  ...
		  service.files.list({
			...
			auth: auth,
			pageSize: 100,
			fields: "nextPageToken, files(id, name, mimeType, parents, properties, description, embedLink)"
		  }, function(err, response) {
			if (err) {
			  console.log('The API returned an error: ' + err);
			  return;
